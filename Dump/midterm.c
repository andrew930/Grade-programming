...
Grade midterm() {
    ...
    results.midexam_grade = calculate_midexam_grade();
    results.parti_grade = calculate_parti_grade();
    results.pt_grade = calculate_pt_grade();
    results.ww_grade = calculate_ww_grade();
    results.proj_grade = calculate_proj_grade();
    results.prob_grade = calculate_prob_grade();
    results.lab_pt_grade = calculate_lab_pt_grade();

    results.total_lec_grade = midexam_grade + parti_grade + pt_grade + ww_grade;
    results.total_lab_grade = proj_grade + prob_grade + lab_pt_grade;
    results.total_midterm_grade = (total_lec_grade*40)/100 + (total_lab_grade*60)/100;

    printf("\nMidterm Term (LEC) Results\n");
    printf("Midterm Examinations (30%): %.2f%%\n", results.midexam_grade);
    printf("Class Participations (20%): %.2f%%\n", results.parti_grade);
    printf("Performance Task (30%): %.2f%%\n", results.pt_grade);
    printf("Written Works (20%): %.2f%%\n", results.ww_grade);
    printf("Midterm (LEC) Grade: %.2f%%\n", results.total_lec_grade);
    
    printf("\nMidterm Term (LAB) Results\n");
    printf("Midterm Project (30%): %.2f%%\n", results.proj_grade);
    printf("Midterm Problems (30%): %.2f%%\n", results.prob_grade);
    printf("Midterm Performance Task (40%): %.2f%%\n", results.lab_pt_grade);
    printf("Midterm (LAB) Grade: %.2f%%\n", results.total_lab_grade);
    
    printf("\nMIDTERM GRADE\n");
    printf("Midterm Grade (MG) = %.2f%%\n", results.total_midterm_grade);
